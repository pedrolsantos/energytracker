...
### CLASSES ###
class FilesHashes():
    def __init__(self, folder, valid_extensions=None, hash_file='hashes.json'):
        self.hash_file = os.path.join(folder, hash_file)
        self.valid_extensions = valid_extensions if valid_extensions is not None else ['.csv']
    ...
    def files_changed(self, folder):
        ...
        new_files_with_valid_ext = any(file.endswith(ext) for file in new_files for ext in self.valid_extensions)

        return new_files_with_valid_ext
...
class DataIngestion():
    ...
    def __init__(self, base_folder, omie_folder, eredes_consumption_folder, eredes_loss_folder, consumption_profile_data, loss_profile_data):
        ...
        self.profiles_data_file = os.path.join(eredes_consumption_folder, self.profiles_data_file)
        self.loss_data_file     = os.path.join(eredes_loss_folder, self.loss_data_file)

        # Verify hashes for OMIE CSV data files....
        self.omie_data = self.checkHash_OMIEData(omie_folder, self.omie_data_file, '.1')
        self.logger.info ( 'OMIE Data Summary:' )
        ...
        # Verify hashes for Consumption Profiles data files....
        self.profile_data = self.checkHash_ConsumptionProfiles(eredes_consumption_folder, self.profiles_data_file, '.xlsx', consumption_profile_data)
        self.logger.info ( 'Consumption Profiles Data Summary:' )
        self.logger.info ( 'Min date = ' + str(self.profile_data.index.min()) )
        self.logger.info ( 'Max date = ' + str(self.profile_data.index.max()) )
        self.logger.info ( 'Total Rows in dataframe = ' + str(len(self.profile_data.index)) )

        # Verify hashes for Loss Profiles data files....
        self.profile_loss_data = self.checkHash_LossProfiles(eredes_loss_folder, self.loss_data_file, '.xlsx', loss_profile_data)
        self.logger.info ( 'Loss Profiles Data Summary:' )
        self.logger.info ( 'Min date = ' + str(self.profile_loss_data.index.min()) )
        self.logger.info ( 'Max date = ' + str(self.profile_loss_data.index.max()) )
        self.logger.info ( 'Total Rows in dataframe = ' + str(len(self.profile_loss_data.index)) )


    def checkHash_OMIEData (self, omie_folder, omie_feather_file, valid_extensions):
        hashesManager = FilesHashes(omie_folder, valid_extensions)
        # Check if the OMIE Data CSV files have changed
        if (not hashesManager.hashfile_created() ) or (hashesManager.files_changed(omie_folder) ) or (not os.path.exists(omie_feather_file)) :
            self.logger.info ('OMIE Data - CSV files changed. Reprocessing all files.....')
            omie_data = self.load_OMIE_data(omie_folder)
            hashesManager.save_hashes(omie_folder)
        else:
            self.logger.info ('OMIE Data - CSV files NOT changed. Loading data from Feather file')
            omie_data = pd.read_feather( omie_feather_file )
            omie_data.set_index('Date', inplace=True)
        return omie_data

    def checkHash_ConsumptionProfiles (self, folder, feather_file, valid_extensions, consumption_profile_template):
        hashesManager = FilesHashes(folder, valid_extensions)
        # Check if Profiles Data files have changed
        if (not hashesManager.hashfile_created() ) or (hashesManager.files_changed(folder) ) or (not os.path.exists(feather_file)) :
            self.logger.info ('Consumption Profile Data - Files changed. Reprocessing all files.....')            
            hashesManager.save_hashes( folder)
        
            # Find all relevant Excel files
            all_files = [os.path.join(folder, f) for f in os.listdir(folder) if f.startswith(consumption_profile_template)]

            self.logger.info (f'Loading E-Redes Consumption Profiles....')

            # Process each file and combine
            all_dfs = [self.load_EREDES_Template_Consumption_file(file) for file in all_files]
            combined_df = pd.concat(all_dfs)

            # Save combined data to Feather file
            combined_df.reset_index().to_feather(feather_file)
            return combined_df
        else:
            self.logger.info ('Consumption Profile Data - Files NOT changed. Loading data from Feather file')
            df = pd.read_feather(feather_file)
            df = df.set_index('Date')
            return df

    def checkHash_LossProfiles (self, folder, feather_file, valid_extensions, loss_profile_template):
        hashesManager = FilesHashes(folder, valid_extensions)
        # Check if Profiles Data files have changed
        if (not hashesManager.hashfile_created() ) or (hashesManager.files_changed(folder) ) or (not os.path.exists(feather_file)) :
            self.logger.info ('Loss Profile Data - Files changed. Reprocessing all files.....')            
            hashesManager.save_hashes( folder)
        
            # Find all relevant Excel files
            all_files = [os.path.join(folder, f) for f in os.listdir(folder) if f.startswith(loss_profile_template)]

            self.logger.info (f'Loading E-Redes Loss Profiles.....')

            # Process each file and combine
            all_dfs = [self.load_EREDES_Template_Loss_file(file) for file in all_files]
            combined_df = pd.concat(all_dfs)
            ...
            self.logger.info ('Loss Profile Data - Files NOT changed. Loading data from Feather file')
            df = pd.read_feather(feather_file)
            df = df.set_index('Date')
            return df
    ...
    def load_EREDES_Template_Consumption_file(self, file_path):
        self.logger.info(f'Loading E-Redes Consumption Profile file= {file_path}')

        df = pd.read_excel(file_path, sheet_name='consumo', skiprows=7, header=None)
    ...
    def load_EREDES_Template_Loss_file(self, file_path, sheet_name='loss'):
        self.logger.info (f'Loading E-Redes Losses Profile file= {file_path}')
        ...
        # convert NaN values in the ATRNT and MAT columns to value 0
        df['ATRNT'] = df['ATRNT'].fillna(0)
        df['MAT'] = df['MAT'].fillna(0)
