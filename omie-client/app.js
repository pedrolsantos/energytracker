...
var CONFIG = {
                ...
                'chart_update_period_net': 23,  // 12 hours default    
                'EoTKey': 'XXXXXXXXXX',
                'cycle_day': 8,
                'profile': 'BTN-C',
                'divisor': 0,
...
class ApiService {
    ...
    async getCurrentPrice(){
      try {
        ...
        const response = await fetch(`${this.baseUrl}/getCurrentPrice?tariff=${tariff}&supplier=${supplier}&cycle_day=${cycle_day}&profile=${profile}`);
    ...
    async getOMIEPricesForPeriod(startDate, endDate ) {
        try {
            ...
            const response = await fetch(
                `${this.baseUrl}/getOMIEPricesForPeriod?tariff=${tariff}&supplier=${supplier}&cycle_day=${cycle_day}&profile=${profile}&start_date=${startDate}&end_date=${endDate}`
            );
    ...
    async uploadEnergyFile(format, provider, file, consumptionType) {
        ...
        const url = `${this.baseUrl}/uploadEnergyFile?supplier=${supplier}&tariff=${tariff}&cycle_day=${cycle_day}&profile=${profile}&format=${format}&provider=${provider}&resampling=${resample}&divisor=${divisor}&lagHour=${lagHour}&datatype=${consumptionType}`;
    ...
    async getEstimationProfile(start_date, end_date, total_energy, records='none') {
        try {
            ...
            const response = await fetch(`${this.baseUrl}/getEstimationProfile?supplier=${supplier}&tariff=${tariff}&cycle_day=${cycle_day}&profile=${profile}&start_date=${start_date}&end_date=${end_date}&total_energy=${total_energy}&records=${records}`);
            if (!response.ok) {
                throw new Error(`HTTP error: ${response.status}`);
            }
            const data = await response.json();
    ...
    async getEstimationProfileManual(start_date, end_date, total_vazio, total_cheio, total_ponta, lagHour=0, records='none' ) {
        try {
            ...
            const response = await fetch(`${this.baseUrl}/getEstimationProfileManual?supplier=${supplier}&tariff=${tariff}&cycle_day=${cycle_day}&profile=${profile}&start_date=${start_date}&end_date=${end_date}&total_vazio=${total_vazio}&total_cheio=${total_cheio}&total_ponta=${total_ponta}&lagHour=${lagHour}&records=${records}`);
            if (!response.ok) {
                throw new Error(`HTTP error: ${response.status}`);
            }
            const data = await response.json();
    ...
    async getEOTData() {
        try {
            ...
            const response = await fetch(`${this.baseUrl}/getEOTData?key=${key}`);
            if (!response.ok) {
                throw new Error(`HTTP error: ${response.status}`);
            }
            const data = await response.json();
...
class PriceTracker {
                    ...
                    tooltip: {
                        callbacks: {
                            title: function (context) {
                                let title = 'Hora: ' + context[0].label ;
                                return title;                            
                            },
    ...
    fetchServerData( priceChart, priceChartNet, force=false) {
        ...
        if (CONFIG.EoTKey !== '' ) {
            this.apiService.getEOTData().then( data => {
                ...
                const power_date    = data['date'];
                const power         = data['power'];
                const energy        = data['energy_total'];
                const cost          = data['total_cost'];
                const month_energy  = data['month_energy_total'];
                const month_cost    = data['month_total_cost'];

                document.getElementById('current-power').textContent = `${power} Watts (${power_date.substring(11,19)})`; 
                document.getElementById('current-consumo-hora').textContent = `${energy.toFixed(2)} kWh (${cost.toFixed(4)} €)`;
                document.getElementById('current-consumo-mensal').textContent = `${month_energy.toFixed(2)} kWh (${month_cost.toFixed(4)} €)`;
            })
            .catch(error => {
                console.error('Error:', error);
            } );
            ...
            document.getElementById('current-consumo-mensal').textContent = `N/A`;
        }
      
        // check if the chart needs to be updated (every 15 minutes)
        const now = new Date();
        const lastUpdateMinutesBlock = Math.floor(this.lastUpdateChart.getMinutes() / 15);
        const currentMinutesBlock = Math.floor(now.getMinutes() / 15);

        if ((now.getHours() !== this.lastUpdateChart.getHours()) || (lastUpdateMinutesBlock !== currentMinutesBlock) || (force === true)) {
    ...
    findIndexByTime(data, targetDate) {
        // Round down the targetDate minutes to the nearest 15-minute block
        const roundedTargetMinutes = Math.floor(targetDate.getMinutes() / 15) * 15;
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (hour === targetHour && roundedMinutes === roundedTargetMinutes) {
                return i;
            }
        }
    ...
    updateChartPeriod(chart, chartnet) {
        ...
        this.apiService.getOMIEPricesForPeriod( this.formatDate(startDate), this.formatDate(endDate))
            .then(data => {                
                ...
                const indexHour = this.findIndexByTime (prices, currentTime);
                ...
                // ****************************************************  OMIE CHART ****************************************************************
                // Set the chart data
                chart.data.labels = prices.map(entry =>DateTime.fromISO(entry.Date).toUTC().toFormat('HH:mm') );
    ...
    loadEnergyChart (chart, prices, profile, oneThird, minCost, maxCost, indexHour) {    
        ...
        var profileHourly = profile; //this.aggregateByHour(profile);
        ...
        if (CONFIG.curva_perfil_c.toLowerCase() === 'true'){
            datasets ={
                labels: prices.map(entry => DateTime.fromISO(entry.Date).toUTC().toFormat('HH:mm')),
