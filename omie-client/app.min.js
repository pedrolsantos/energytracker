...
if(!c.ok)throw Error(`HTTP error: ${c.status}`);return await c.json()}catch(c){throw console.error(`Error fetching OMIE prices for period: ${c}`),c;}}async uploadEnergyFile(a,b,c){a=`${this.baseUrl}/uploadEnergyFile?supplier=${CONFIG.supplier}&tariff=${CONFIG.tariff}&year=${CONFIG.year}&cycle_day=${CONFIG.cycle_day}&profile=${CONFIG.profile}&format=${a}&provider=${b}&resampling=${sample}&divisor=${0!=CONFIG.divisor?CONFIG.divisor:""}&lagHour=${0}`;b=new FormData;b.append("file",c);c={method:"POST",
body:b};try{const d=await fetch(a,c);if(!d.ok)throw Error(`HTTP error! status: ${d.status}`);return await d.json()}catch(d){throw console.error("Error uploading file:",d),d;}}async getEstimationProfile(a,b,c,d="none"){try{const e=await fetch(`${this.baseUrl}/getEstimationProfile?supplier=${CONFIG.supplier}&tariff=${CONFIG.tariff}&year=${CONFIG.year}&cycle_day=${CONFIG.cycle_day}&profile=${CONFIG.profile}&start_date=${a}&end_date=${b}&total_energy=${c}&records=${d}`);if(!e.ok)throw Error(`HTTP error: ${e.status}`);
return await e.json()}catch(e){throw console.error(`Error fetching estimation profile: ${e}`),e;}}async getEstimationProfileManual(a,b,c,d,e,g=0,k="none"){try{const h=await fetch(`${this.baseUrl}/getEstimationProfileManual?supplier=${CONFIG.supplier}&tariff=${CONFIG.tariff}&year=${CONFIG.year}&cycle_day=${CONFIG.cycle_day}&profile=${CONFIG.profile}&start_date=${a}&end_date=${b}&total_vazio=${c}&total_cheio=${d}&total_ponta=${e}&lagHour=${g}&records=${k}`);if(!h.ok)throw Error(`HTTP error: ${h.status}`);
