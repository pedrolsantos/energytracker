var CONFIG={api_url:"",tariff:"Tri-Horario-Semanal",curva_perfil_c:"True",posicao_grafico:"meio",supplier:"coopernico-base",chart_update_interval:10,chart_update_period:23,chart_update_period_net:23,EoTKey:"XXXXXXXXXX",cycle_day:8,profile:"BTN-C",divisor:0,allowDebug:!1};
...
class PriceTracker{constructor(a,b,c="meio"){this.apiService=a;this.chartUpdatePeriod=b;this.chartPosition=c;this.lastUpdateChart=new Date;this.analysisChart=this.lastEstimateDataDisplay=this.lastRealDataDisplay=this.lastEstimateDataFetch=this.lastRealDataFetch=null;this.isRealConsumption=!0;this.zoomOptions={pan:{enabled:!0,mode:"x"},zoom:{wheel:{enabled:!0},pinch:{enabled:!0},drag:{enabled:!1},mode:"x"}};this.chartConfig={type:"bar",data:{labels:[],datasets:[{label:"Energia",data:[],borderColor:"rgba(75, 192, 192, 1)",
backgroundColor:"rgba(75, 192, 192, 0.2)",borderWidth:1,tension:.1}]},options:{responsive:!0,maintainAspectRatio:!1,scales:{x:{title:{display:!0,text:"Hora"}},y:{title:{display:!0,text:"Pre\u00e7o"}}},plugins:{tooltip:{callbacks:{title:function(d){return"Hora: "+d[0].label},label:function(d){let e=d.dataset.label||"";e&&="Pre\u00e7o: ";return e+=Number(d.parsed.y).toFixed(2)+" \u20ac/MWh"}}}}}};this.chartConfigNet={type:"bar",data:{labels:[],datasets:[{label:"Energia",data:[],borderColor:"rgba(75, 192, 192, 1)",
backgroundColor:"rgba(75, 192, 192, 0.2)",borderWidth:1,tension:.1}]},options:{responsive:!0,maintainAspectRatio:!1,scales:{x:{title:{display:!0,text:"Hora"}},y:{title:{display:!0,text:"Pre\u00e7o"}}},plugins:{tooltip:{callbacks:{title:function(d){return"Hora: "+d[0].label},label:function(d){let e=d.dataset.label||"";e&&="Pre\u00e7o: ";return e+=Number(d.parsed.y).toFixed(4)+" \u20ac/kWh"}}}}}}}initCharts(){var a=document.getElementById("price-chart");a=new Chart(a,this.chartConfig);var b=document.getElementById("price-chart-net");
...
const e=new Date,h=new Date;e.setMinutes(0,0,0);let f;switch(this.chartPosition){case "meio":f=.5*d;var k=.5*d;break;case "esquerda":f=.1*d;k=.9*d;break;case "direita":f=.8*d;k=.2*d;break;default:console.error("updateChartPeriod:: Invalid chart position")}d=new Date(e.getTime()-36E5*f);k=new Date(e.getTime()+36E5*k);logger("Fetching period from:["+d+"] to:["+k+"] date=["+e+"]");this.apiService.getOMIEPricesForPeriod(this.formatDate(d),this.formatDate(k)).then(g=>{logger("OMIE Prices:",g);var m=g.prices;
g=g.profile;const p=this.findIndexByTime(m,h),l=Math.min(...m.map(n=>n.Price_PT)),q=Math.max(...m.map(n=>n.Price_PT)),u=Math.min(...m.map(n=>n.Cost)),v=Math.max(...m.map(n=>n.Cost));a.data.labels=m.map(n=>c.fromISO(n.Date).toUTC().toFormat("HH:mm"));a.data.datasets[0].label="Energia OMIE";a.data.datasets[0].data=m.map(n=>n.Price_PT);let t=(q-l)/3;a.data.datasets[0].backgroundColor=m.map((n,r)=>this.getColor(n.Price_PT,l,t,r==p));a.data.datasets[0].borderColor=m.map((n,r)=>r==p?"darkblue":"lightgray");
a.update();this.loadEnergyChart(b,m,g,t,u,v,p)}).catch(g=>{console.error("Error:",g)});return e}getMinMaxDate(a){if(0===a.length)return{minDate:null,maxDate:null};let b=new Date(a[0].Date),c=new Date(a[0].Date);for(const d of a)a=new Date(d.Date),a<b&&(b=a),a>c&&(c=a);return{minDate:b,maxDate:c}}processEnergyData(a){let b={vazioCost:0,cheioCost:0,pontaCost:0,totalCost:0,vazioEnergy:0,cheioEnergy:0,pontaEnergy:0,vazioAvgPrice:0,cheioAvgPrice:0,pontaAvgPrice:0,totalEnergy:0,globalAvgPrice:0};a.forEach(c=>
{null!==c.Vazio&&(b.vazioCost+=c.Cost,b.vazioEnergy+=c.Vazio);null!==c.Cheio&&(b.cheioCost+=c.Cost,b.cheioEnergy+=c.Cheio);null!==c.Ponta&&(b.pontaCost+=c.Cost,b.pontaEnergy+=c.Ponta);b.totalCost+=c.Cost});b.vazioAvgPrice=b.vazioEnergy?b.vazioCost/b.vazioEnergy:0;b.cheioAvgPrice=b.cheioEnergy?b.cheioCost/b.cheioEnergy:0;b.pontaAvgPrice=b.pontaEnergy?b.pontaCost/b.pontaEnergy:0;b.totalEnergy=b.vazioEnergy+b.cheioEnergy+b.pontaEnergy;b.globalAvgPrice=b.totalCost/b.totalEnergy;return b}getTotalEnergyCost(a){return a.reduce((b,
c)=>({sumEnergy:b.sumEnergy+c.Energy,sumCost:b.sumCost+c.Cost,sumVazio:b.sumVazio+c.Vazio,sumCheio:b.sumCheio+c.Cheio,sumPonta:b.sumPonta+c.Ponta}),{sumEnergy:0,sumCost:0,sumVazio:0,sumCheio:0,sumPonta:0})}updateChartAnalysis(a,b){document.getElementById("title_consumo_real").textContent="Consumo Real :";const c=this.getTotalEnergyCost(b);var d=this.processEnergyData(b);const e=this.getMinMaxDate(b);var h=this.getMinMaxDate(this.lastRealDataFetch);c.sumEnergy.toFixed(2);document.getElementById("analysis-periodo-start").value=
this.formatDate(h.minDate,!0);document.getElementById("analysis-periodo-end").value=this.formatDate(h.maxDate,!0);document.getElementById("analysis-custo-total").textContent=`${d.totalCost.toFixed(2)}`+" \u20ac";document.getElementById("analysis-custo-preco-medio").textContent=`${d.globalAvgPrice.toFixed(6)} `+" \u20ac/kWh";document.getElementById("analysis-energia-vazio-total").textContent=`${d.vazioEnergy.toFixed(2)} `+" kWh ("+`${d.vazioCost.toFixed(2)} `+" \u20ac)";document.getElementById("analysis-energia-cheio-total").textContent=
`${d.cheioEnergy.toFixed(2)} `+" kWh ("+`${d.cheioCost.toFixed(2)} `+" \u20ac)";document.getElementById("analysis-energia-ponta-total").textContent=`${d.pontaEnergy.toFixed(2)} `+" kWh ("+`${d.pontaCost.toFixed(2)} `+" \u20ac)";document.getElementById("analysis-energia-total").textContent=`${d.totalEnergy.toFixed(2)} `+" kWh";document.getElementById("analysis-custo-preco-medio-vazio").textContent=`${d.vazioAvgPrice.toFixed(6)} `+" \u20ac/kWh";document.getElementById("analysis-custo-preco-medio-cheio").textContent=
`${d.cheioAvgPrice.toFixed(6)} `+" \u20ac/kWh";document.getElementById("analysis-custo-preco-medio-ponta").textContent=`${d.pontaAvgPrice.toFixed(6)} `+" \u20ac/kWh";document.getElementById("title_profile").textContent="Curva Perfil "+CONFIG.profile+" ..... Em Processamento ....";d=document.getElementById("analysis-contador-vazio").value;h=document.getElementById("analysis-contador-cheio").value;var f=document.getElementById("analysis-contador-ponta").value;0==d&&(d=c.sumVazio.toFixed(2),document.getElementById("analysis-contador-vazio").value=
d);0==h&&(h=c.sumCheio.toFixed(2),document.getElementById("analysis-contador-cheio").value=h);0==f&&(f=c.sumPonta.toFixed(2),document.getElementById("analysis-contador-ponta").value=f);this.fetchProfileDataBasedOnManual(e,d,h,f);this.lastRealDataDisplay=b;document.getElementById("switch-chart").checked&&(this.isRealConsumption=!0,this.loadDataOnAnalysisChart(a,this.lastRealDataDisplay))}aggregateByHour(a){const b={};a.forEach(c=>{var d=new Date(c.Date);d=(new Date(d.getFullYear(),d.getMonth(),d.getDate(),
d.getHours())).toISOString();b[d]||(b[d]={Date:d,Energy:0,Vazio:null,Cheio:null,Ponta:null});null!=c.Vazio&&(b[d].Vazio+=c.Vazio);null!=c.Cheio&&(b[d].Cheio+=c.Cheio);null!=c.Ponta&&(b[d].Ponta+=c.Ponta);b[d].Energy+=c.Energy;var e=[b[d].Vazio,b[d].Cheio,b[d].Ponta].filter(h=>null!==h);0<e.length&&(c=Math.max(...e),e=Math.min(...e),b[d].Vazio==c?b[d].Cheio==e?(b[d].Vazio+=e,b[d].Cheio=null):b[d].Ponta==e&&(b[d].Vazio+=e,b[d].Ponta=null):b[d].Cheio==c?b[d].Vazio==e?(b[d].Cheio+=e,b[d].Vazio=null):
b[d].Ponta==e&&(b[d].Cheio+=e,b[d].Ponta=null):b[d].Ponta==c&&(b[d].Vazio==e?(b[d].Ponta+=e,b[d].Vazio=null):b[d].Cheio==e&&(b[d].Ponta+=e,b[d].Cheio=null)))});return Object.values(b)}loadEnergyChart(a,b,c,d,e,h,f){const k=luxon.DateTime;d=(h-e)/3;h={type:"bar",label:"Energia "+(CONFIG.supplier.toLowerCase().includes("coopernico")?"Coopernico":"LuzBoa"),data:b.map(l=>l.Cost),backgroundColor:b.map((l,q)=>this.getColor(l.Cost,e,d,q==f)),borderColor:b.map((l,q)=>q==f?"darkblue":"lightgray"),borderWidth:1,
yAxisID:"yAxisCost"};var g={type:"line",label:"Vazio",data:c.map(l=>l.Vazio),backgroundColor:"rgba(84, 234, 9, 0.8)",borderWidth:1,yAxisID:"yAxisConsumo"},m={type:"line",label:"Cheio",data:c.map(l=>l.Cheio),backgroundColor:"rgba(15, 225, 225, 0.9)",borderWidth:1,yAxisID:"yAxisConsumo"};c={type:"line",label:"Ponta",data:c.map(l=>l.Ponta),backgroundColor:"rgba(15, 120, 225, 0.9)",borderWidth:1,yAxisID:"yAxisConsumo"};var p={};p="true"===CONFIG.curva_perfil_c.toLowerCase()?{labels:b.map(l=>k.fromISO(l.Date).toUTC().toFormat("HH:mm")),
datasets:[h,g,m,c]}:{labels:b.map(l=>k.fromISO(l.Date).toUTC().toFormat("HH:mm")),datasets:[h]};a.data=p;a.options={responsive:!0,maintainAspectRatio:!1,scales:{x:{title:{display:!0,text:"Hora"}},yAxisConsumo:{position:"right",display:!1,title:{display:!1,text:"Consumo (kWh)"}},yAxisCost:{position:"left",title:{display:!0,text:"Custo (\u20ac/kWh)"}}},plugins:{tooltip:{callbacks:{title:function(l){return"Hora: "+l[0].label},label:function(l){let q="";return q=0==l.datasetIndex?q+("Custo: "+Number(l.parsed.y).toFixed(4)+
" \u20ac"):q+("Quantidade: "+Number(l.parsed.y).toFixed(4)+" kWh")}}}}};a.update();a.resize()}loadDataOnAnalysisChart(a,b){var c={type:"line",label:"Consumo Vazio",data:b.map(g=>g.Vazio),backgroundColor:"rgba(84, 234, 9, 0.8)",borderWidth:1,yAxisID:"yAxisConsumo"},d={type:"line",label:"Consumo Cheio",data:b.map(g=>g.Cheio),backgroundColor:"rgba(15, 225, 225, 0.9)",borderWidth:1,yAxisID:"yAxisConsumo"},e={type:"line",label:"Consumo Ponta",data:b.map(g=>g.Ponta),backgroundColor:"rgba(15, 120, 225, 0.9)",
borderWidth:1,yAxisID:"yAxisConsumo"},h={type:"bar",label:"Custo",data:b.map(g=>g.Cost),backgroundColor:"rgba(241, 163, 6, 0.5)",borderColor:"rgba(241, 163, 6, 0.5)",borderWidth:1,yAxisID:"yAxisCost"};const f={pan:{enabled:!0,mode:"x"},zoom:{wheel:{enabled:!0},pinch:{enabled:!0},drag:{enabled:!1},mode:"x"}};var k={responsive:!0,maintainAspectRatio:!1,scales:{yAxisConsumo:{position:"left",title:{display:!0,text:"Consumo (kWh)"}},yAxisCost:{position:"right",title:{display:!0,text:"Custo (\u20ac/kWh)"}}},
plugins:{zoom:f,title:{display:!0,position:"bottom",text:g=>"Zoom: "+((f.zoom.wheel.enabled?"enabled":"disabled")+" ("+g.chart.getZoomLevel()+"x), Pan: ")+(f.pan.enabled?"enabled":"disabled")},tooltip:{callbacks:{label:function(g){return 3==g.datasetIndex?"Custo: "+Number(g.parsed.y).toFixed(4)+" \u20ac":"Quantidade: "+Number(g.parsed.y).toFixed(4)+" kWh"}}}}};b={labels:b.map(g=>g.Date),datasets:[c,d,e,h]};a.data=b;a.options=k;a.update();a.resize()}setupSliderForData(a,b){this.getMinMaxDate(b);var c=
b.length-1;this.analysisChart=a;this.lastRealDataDisplay=this.lastRealDataFetch=b;document.getElementById("analysis-slider-periodo").textContent=b[0].Date+"    -    "+b[c].Date;b={id:"slider_chart",enabled:!0,tooltip:"hide",tooltip_split:!1,min:0,max:c,step:1,value:[0,c]};slider.getElement();slider.destroy();slider=new Slider("#analysis-data-range",b);slider.enable();c=this.lastRealDataDisplay.slice(0,c+1);this.updateChartAnalysis(a,c);slider.on("slideStop",this.onSlideStop);slider.on("change",this.onSlideChange)}onSlideChange(a){var b=
a.newValue;if(Array.isArray(b)){const c=priceTrackerApp.lastRealDataFetch;a=b[0];b=b[1];document.getElementById("analysis-slider-periodo").textContent=c[a].Date+"    -    "+c[b].Date}}onSlideStop(a){const b=priceTrackerApp.lastRealDataFetch.slice(a[0],a[1]+1);priceTrackerApp.updateChartAnalysis(priceTrackerApp.analysisChart,b);document.getElementById("title_profile").textContent="Curva Perfil "+CONFIG.profile+" ..... Em Processamento ....";logger("SLIDING",a,b)}getCurrentDataForSlider(){var a=this.lastRealDataFetch,
b=priceTrackerApp.getMinMaxDate(a);return[a,priceTrackerApp.formatDate(b.minDate,!0),priceTrackerApp.formatDate(b.maxDate,!0)]}fetchProfileDataBasedOnManual(a,b,c,d){apiService.getEstimationProfileManual(this.formatDate(a.minDate),this.formatDate(a.maxDate),b,c,d,0,"json").then(e=>{const h=e.Total_Cost/e.Total_energy;document.getElementById("analysis-energia-profile-vazio-total").textContent=`${e.Energy_Vazio.toFixed(2)} `+" kWh ("+`${e.Cost_Vazio.toFixed(2)} `+" \u20ac)";document.getElementById("analysis-energia-profile-cheio-total").textContent=
...
document.addEventListener("DOMContentLoaded",()=>{var a=localStorage.getItem("config");a&&(a=JSON.parse(a),document.getElementById("config-perfil").value=void 0===a.curva_perfil_c?CONFIG.curva_perfil_c:a.curva_perfil_c,document.getElementById("config-hours").value=void 0===a.hours?CONFIG.chart_update_period:a.hours,document.getElementById("config-posicao-grafico").value=void 0===a.posicao_grafico?CONFIG.posicao_grafico:a.posicao_grafico,document.getElementById("config-tariff").value=void 0===a.tariff?
CONFIG.tariff:a.tariff,document.getElementById("config-supplier").value=void 0===a.supplier?CONFIG.supplier:a.supplier,document.getElementById("config-eot-key").value=void 0===a.configEotKey?CONFIG.EoTKey:a.configEotKey,document.getElementById("config-cycle-day").value=void 0===a.cycle_day?CONFIG.cycle_day:a.cycle_day,document.getElementById("config-profile").value=void 0===a.profile?CONFIG.profile:a.profile,document.getElementById("config-refresh-rate").value=void 0===a.chart_update_interval?CONFIG.chart_update_interval:
a.chart_update_interval,CONFIG.chart_update_interval=void 0===a.chart_update_interval?CONFIG.chart_update_interval:a.chart_update_interval,CONFIG.chart_update_period=void 0===a.hours?CONFIG.chart_update_period:a.hours,CONFIG.chart_update_period_net=void 0===a.hours?CONFIG.chart_update_period_net:a.hours,CONFIG.posicao_grafico=void 0===a.posicao_grafico?CONFIG.posicao_grafico:a.posicao_grafico,CONFIG.tariff=void 0===a.tariff?CONFIG.tariff:a.tariff,CONFIG.supplier=void 0===a.supplier?CONFIG.supplier:
a.supplier,CONFIG.EoTKey=void 0===a.configEotKey?CONFIG.EoTKey:a.configEotKey,CONFIG.curva_perfil_c=void 0===a.curva_perfil_c?CONFIG.curva_perfil_c:a.curva_perfil_c,CONFIG.cycle_day=void 0===a.cycle_day?CONFIG.cycle_day:a.cycle_day,CONFIG.profile=void 0===a.profile?CONFIG.profile:a.profile,logger("Loaded CONFIG: ",CONFIG));a=window.location.protocol;var b=window.location.hostname;"localhost"===b||"file:"===window.location.protocol?(CONFIG.api_url="http://localhost:15000",CONFIG.allowDebug=!0):(CONFIG.api_url=
`${a}//${b}${":15000"}`,CONFIG.allowDebug=!1);logger("Starting the app... ADDRESS:",CONFIG.api_url);apiService=new ApiService(CONFIG.api_url);priceTrackerApp=new PriceTracker(apiService,CONFIG.chart_update_period,CONFIG.posicao_grafico);[priceChart,priceChartNet,priceChartAnalysis]=priceTrackerApp.initCharts();document.getElementById("consumo-browse-file").addEventListener("change",handleFileInputChange);document.getElementById("analysis-contador-refresh").addEventListener("click",handleAnaliseRefreshContador);
document.getElementById("switch-chart").addEventListener("change",handleSwitchChange_Chart);document.getElementById("configForm").addEventListener("submit",handleStoreConfig);document.getElementById("title_profile").textContent="Curva Perfil "+CONFIG.profile;slider=new Slider("#analysis-data-range",{enabled:!1});priceTrackerApp.setAnalysisPageforSupplier(CONFIG.supplier,CONFIG.tariff);priceTrackerApp.fetchServerData(priceChart,priceChartNet,force=!0);resetTimer();document.getElementById("btn-reset-zoom").addEventListener("click",
...
"default")});document.getElementById("btn-resample-15m").addEventListener("click",function(){sample="";file&&uploadFile(file)});document.getElementById("btn-resample-1h").addEventListener("click",function(){sample="1H";file&&uploadFile(file)});document.getElementById("btn-resample-1d").addEventListener("click",function(){sample="1D";file&&uploadFile(file)});document.getElementById("btn-resample-1w").addEventListener("click",function(){sample="1W";file&&uploadFile(file)});a=["analysis-contador-vazio",
"analysis-contador-cheio","analysis-contador-ponta"];for(var c of a){const d=document.getElementById(c);d.addEventListener("change",function(){validateInput(d)})}a=new Date;c=new Date(a.getFullYear(),a.getMonth(),a.getDate()-2,0,0);a=new Date(a.getFullYear(),a.getMonth(),a.getDate()-1,23,59);b=document.getElementById("analysis-periodo-start");flatpickr(b,{enableTime:!0,dateFormat:"Y-m-d H:i",defaultDate:c,time_24hr:!0});c=document.getElementById("analysis-periodo-end");flatpickr(c,{enableTime:!0,
dateFormat:"Y-m-d H:i",defaultDate:a,time_24hr:!0})});function logger(...a){var b=!1;const c=localStorage.getItem("config");c&&(b=JSON.parse(c),b=void 0===b.debug?CONFIG.allowDebug:b.debug);b&&console.log(...a)}function resetTimer(){timerId&&clearInterval(timerId);timerId=setInterval(()=>{priceTrackerApp.fetchServerData(priceChart,priceChartNet)},6E4*CONFIG.chart_update_interval)}
function handleStoreConfig(a){a.preventDefault();a=document.getElementById("config-hours").value;const b=document.getElementById("config-perfil").value,c=document.getElementById("config-posicao-grafico").value,d=document.getElementById("config-tariff").value,e=document.getElementById("config-supplier").value,h=document.getElementById("config-eot-key").value,f=document.getElementById("config-cycle-day").value,k=document.getElementById("config-profile").value,g=document.getElementById("config-refresh-rate").value,
m=CONFIG.api_url,p={apiUrl:m,hours:a,posicao_grafico:c,tariff:d,configEotKey:h,supplier:e,curva_perfil_c:b,cycle_day:f,profile:k,chart_update_interval:g};CONFIG.chart_update_period=a;CONFIG.chart_update_period_net=a;CONFIG.posicao_grafico=c;CONFIG.tariff=d;CONFIG.supplier=e;CONFIG.EoTKey=h;CONFIG.curva_perfil_c=b;CONFIG.cycle_day=f;CONFIG.profile=k;CONFIG.chart_update_interval=g;priceTrackerApp.apiService=new ApiService(m);priceTrackerApp.chartUpdatePeriod=a;priceTrackerApp.chartUpdatePeriodNet=a;
priceTrackerApp.chartPosition=c;localStorage.setItem("config",JSON.stringify(p));logger("Configuration saved!",CONFIG);resetTimer();priceTrackerApp.setAnalysisPageforSupplier(CONFIG.supplier,CONFIG.tariff);priceTrackerApp.fetchServerData(priceChart,priceChartNet,force=!0);(new bootstrap.Tab(document.getElementById("home-tab"))).show()}
